...
namespace BLL.Services.ImplementationService.Admin
{
    public class AdminService : IAdminService
    {
        ...
        public async Task<AdminUserDto> DeleteUserAsync(int userId, int requestingAdminId)
        {
            ...
            {
                throw new BadRequestException(new System.Collections.Generic.List<string> { "This account cannot be deleted because it has linked records (appointments, schedules, etc.). Please deactivate the account instead." });
            }
        ...
        public async Task<object> GetSpecialtiesAsync()
        {
            return await _userRepository.GetDistinctSpecialtiesAsync();
        }

        public async Task<AdminUserDto> ToggleUserActiveStatusAsync(int userId, int requestingAdminId)
        {
            var user = await _userManager.FindByIdAsync(userId.ToString());
            if (user == null)
                throw new UserByIdNotFoundException(userId);
            ...
            if (userId == requestingAdminId)
                throw new BadRequestException(new System.Collections.Generic.List<string> { "Admins cannot toggle their own active status." });

            var isTargetAdmin = await _userManager.IsInRoleAsync(user, "Admin");
            if (isTargetAdmin)
                throw new BadRequestException(new System.Collections.Generic.List<string> { "Cannot toggle status of another Admin account." });
